...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println( "sum2ndRow = " + sum2ndRow( arr ) );  // sum( arr[1] ) = { 2 , 0, 67 } = 69
    }
/*
...
findMax               - Реализовать метод findMax(int[][] array) который должен найти и вернуть максимальный элемент массива;
sum2ndRow             - Реализуйте метод, который считает сумму элементов второй строки двумерного массива, если второй строки не существует, то в качестве результата необходимо вернуть -1
 ...
 */
    ...
    public static void zeroarr( int[][] arr ) {
        int sz = Math.min(arr.length,arr[0].length);
        for (int i = 0; i < sz; i++) {
            arr[i][i] = 0; // зануляем диагональ обратный слэш
        }
        for (int i = 0; i < sz; i++) {
            arr[i][sz-1 -i] = 0; // зануляем вторую диагональ (прямой слэш)
        }
    }
    ...
    public static int sum2ndRow( int[][] arr ) {
        int sum = 0;
        if (arr.length > 2) { // если вторая строка существует
